...
Milestone 9 is split into 9A preparation and 9B guarded execution. 9A adds ignored private roots, bounded discovery, explicit assignments, safe archive handling, source-profile dry runs, mapping/checkpoint/reconciliation contracts, dedicated-database ownership/write gates, and sanitized missing-input reports. 9B remains blocked until every file, database, protocol, Git, migration, and explicit-write prerequisite passes; it never performs final predictive training or causal estimation.

...
public enum ImportSourceKind { Fda, Medicaid, StateReference }
public enum ImportRowDisposition { Accepted, Rejected, Duplicate, ExplicitlyExcluded }
public sealed record ImportSourceRegistration(string RelativePath,string Sha256,long ByteSize,string SchemaVersion,ImportSourceKind Kind,long ExpectedRows,bool Canonical);
...
public sealed record ImportInitialization(string DatasetVersionCode,long ProtocolId,string CorrelationId,string ManifestHash,IReadOnlyList<ImportSourceRegistration> Sources,string GitCommit);
...
public sealed class ResearchImportCoordinator(IResearchImportPersistence persistence)
{
    ...
    {
        if(batchSize<=0)throw new ArgumentOutOfRangeException(nameof(batchSize)); var ordered=initialization with{Sources=initialization.Sources.Where(x=>x.Canonical).OrderBy(x=>x.Kind).ThenBy(x=>x.RelativePath,StringComparer.Ordinal).ToArray()};var session=await persistence.InitializeAsync(ordered,cancellationToken);long registered=0,accepted=0,rejected=0,duplicate=0,excluded=0;var batchNumber=0;

...
public sealed class ResearchImportExecutionService(PharmaAccessDbContext db)
{
    ...
    {
        var initialization=await PrepareAsync(privateRoot,manifestPath,validationPath,protocolCode,protocolVersion,datasetVersion,correlationId,gitCommit,resume,ct);
        return await new ResearchImportCoordinator(new EfResearchImportPersistence(db)).ExecuteAsync(initialization,ReadBatches(privateRoot,initialization.Sources.Where(x=>x.Canonical).ToArray(),batchSize,ct),batchSize,ct);
    }
    ...
    {
        var initialization=await PrepareAsync(privateRoot,manifestPath,validationPath,protocolCode,protocolVersion,datasetVersion,correlationId,gitCommit,false,ct);
        return ResearchImportInitializationFactory.Build(initialization,DateTime.UtcNow);
    }

    private async Task<ImportInitialization> PrepareAsync(string privateRoot,string manifestPath,string validationPath,string protocolCode,string protocolVersion,string datasetVersion,string correlationId,string gitCommit,bool resume,CancellationToken ct)
    {
        ...
        return new(datasetVersion,protocol.ResearchProtocolId,correlationId,Convert.ToHexString(SHA256.HashData(Encoding.UTF8.GetBytes(manifestText))),sources,gitCommit);
    }

...
public sealed class EfModelGovernanceRepository(PharmaAccessDbContext db) : IModelGovernanceRepository
{
    ...
    private static ChampionChallengerComparison Map(GovernanceComparison x){var envelope=JsonSerializer.Deserialize<PersistedComparisonEnvelope>(x.ChallengerJson);var challenger=envelope?.Snapshot??JsonSerializer.Deserialize<GovernedModelSnapshot>(x.ChallengerJson)!;return new(x.ComparisonId,JsonSerializer.Deserialize<GovernedModelSnapshot>(x.ChampionJson)!,challenger,x.CreatedAtUtc,JsonSerializer.Deserialize<MetricDifference[]>(x.MetricDifferencesJson)??[],JsonSerializer.Deserialize<string[]>(x.SubgroupResultsJson)??[],JsonSerializer.Deserialize<string[]>(x.BlockingReasonsJson)??[],x.PromotionEligible,"Persisted human-governed comparison.",envelope?.SubmitterIdentifier??"");}
    private sealed record PersistedComparisonEnvelope(GovernedModelSnapshot Snapshot,string SubmitterIdentifier);
